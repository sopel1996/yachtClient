...
export const WorkEdit = ({}) => {
  ...
  const onSubmitHandler = (event, params)=>{
    ...
    const {
      ...
      console.log(firstWorkDate)
    console.log('nextWorkDate',nextWorkDate)
    work.append("workName", workName);
    work.append("category", category);
    work.append("description", description);
    work.append("oneTimeJob", oneTimeJob);
    work.append("period", period);
    work.append("firstWorkDate", firstWorkDate.format('YYYY-MM-DD'));
    work.append("nextWorkDate", firstWorkDate.format('YYYY-MM-DD'));
  
    fetch(`http://localhost:5000/api/work/${id}/edit`, {
      method: "PUT",
      
      body: work,
    })
      .then((response) => navigate(`/works/${id}`))
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:5000/api/work/${params.id}`, {
      ...
      .then((result) => {
        ...
        setOneTimeJob(work.oneTimeJob)
        setPeriod(work.period)
        console.log(dayjs(work.firstWorkDate, "YYYY-MM-DD").format("DD.MM.YYYY"))
        setFirstWorkDate(dayjs(work.firstWorkDate, "YYYY-MM-DD"))
